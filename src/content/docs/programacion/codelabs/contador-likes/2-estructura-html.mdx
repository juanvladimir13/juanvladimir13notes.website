---
title: '3. Estructura de un documento HTML'
description: 'Creación del documento HTML base, enlace de scripts JavaScript y verificación de ejecución inicial en el navegador.'

head:
  - tag: meta
    attrs:
      property: og:title
      content: '3. Estructura de un documento HTML'
  - tag: meta
    attrs:
      property: og:description
      content: 'Creación del documento HTML base, enlace de scripts JavaScript y verificación de ejecución inicial en el navegador.'
  - tag: meta
    attrs:
      property: og:image
      content: 'https://juanvladimir13codelabs.web.app/og-programming.jpg'
  - tag: meta
    attrs:
      property: og:image:width
      content: '1200'
  - tag: meta
    attrs:
      property: og:image:height
      content: '630'
  - tag: meta
    attrs:
      property: og:locale
      content: 'es_BO'
  - tag: meta
    attrs:
      property: og:type
      content: 'article'
  - tag: meta
    attrs:
      property: twitter:card
      content: 'summary_large_image'
  - tag: meta
    attrs:
      property: twitter:title
      content: '3. Estructura de un documento HTML'
  - tag: meta
    attrs:
      property: twitter:description
      content: 'Creación del documento HTML base, enlace de scripts JavaScript y verificación de ejecución inicial en el navegador.'
  - tag: meta
    attrs:
      property: twitter:image
      content: 'https://juanvladimir13codelabs.web.app/og-programming.jpg'
---

import Keycap from '@components/Keycap.astro';

```html title="index.html" showLineNumbers
<html lang="es">
  <head>
    <meta charset="UTF-8">
    <title>Contador</title>
  </head>
  <body>
    <header></header>
    <main>
      <h1>
        Contador de likes
      </h1>
    </main>
    <footer></footer>
  </body>
</html>
```

### Incluir archivos scripts en HTML
Agregar el archivo `counter.js` al archivo `index.html`, incluir el siguiente contenido antes de la etiqueta `</body>`

```html title="index.html" 
<script src="counter.js"></script>
```

## Funciones de javascript/typescript

```js title="counter.js"
alert("Hola mundo");
```

### Verificar avance
Abrir el archivo `index.html` en el navegador **google chrome**

#### Resultado correcto 😎
![Resultado correcto](./../../../../../assets/docs/contador-likes/hola-mundo.png)


#### Resultado incorrecto ☹️
- Presionar la combinacion de teclas <Keycap text="ctrl" /> + <Keycap text="shiftName" /> + <Keycap text="i" />
- Seleccionar el menu `Console`
- Leer los mensajes de error y solucionar el **bug** o **typo**

![Resultado incorrecto](./../../../../../assets/docs/contador-likes/console-error.png)

:::note
**Bug** es un programa que no hace lo que se espera que haga

**Typo** es un error de tipeo (escritura del codigo)
:::
